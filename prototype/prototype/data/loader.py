...
def load_csv(source: TextIO, name: str) -> Relation:
    ...
    Type inference is applied per column: int > float > bool > str.
...
def infer_types(rows: list[dict[str, str]]) -> dict[str, type]:
    ...
    A column is float if every non-empty value parses as float (but not all int).
...
def _is_float(s: str) -> bool:
    """Check if a string is a valid float literal."""
    try:
        float(s)
        return True
    except ValueError:
